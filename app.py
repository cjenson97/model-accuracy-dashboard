...
def filter_by_week(df, date_col, week_start, week_end):
    ...
    week_start_ts = pd.Timestamp(week_start).tz_localize(None)
    week_end_ts = pd.Timestamp(week_end).tz_localize(None)
    return df[(dates >= week_start_ts) & (dates <= week_end_ts)]
...
# Build list of weeks in the selected month
...
with tab1:
    ...
    st.caption(
        "Source: **feedback_loop_prod** table — records every update the model scored "
    ...
    if df_feedback.empty:
        ...
        with col_b:
            ...
            st.caption(
                f"3s accuracy and 1s accuracy side by side. "
        ...
        st.caption(
            "Score-3 and score-2 updates that an analyst has not yet reviewed. "
            "A large bar means that industry has a backlog of unreviewed updates — "
            "accuracy figures for that industry may be incomplete."
        )
...
with tab2:
    ...
    if df_feedback.empty:
        ...
        if df_w.empty:
            ...
            sel = st.multiselect(
                "Filter by industry", industries, default=industries[:6]
            )
            ...
            with col_a:
                ...
                st.caption(
                    f"Each line is one industry tracked across weeks. "
                    f"The '── Benchmark' line is {BENCHMARK_3S}%. "
                    "Lines below benchmark = model is over-scoring updates as high-relevance."
                )
            ...
            with col_b:
                ...
                st.caption(
                    f"Each line is one industry tracked across weeks. "
                    f"The '── Benchmark' line is {BENCHMARK_1S}%. "
                    "Lines below benchmark = model is incorrectly flagging updates as low-relevance."
                )
...
with tab3:
    ...
    st.caption(
        ...
        "This does not include analyst feedback; it shows what the model scored and how confident it was. "
        "Use this to explore individual updates or audit the model's output."
    )
    ...
    if df_scored.empty:
        ...
        with col_a:
            ...
            st.caption(
                "How the model distributed scores across all updates in this period. "
                "Score 3 = high relevance, 2 = medium, 1 = low. "
                "A healthy spread should lean towards 1s and 3s."
            )
        ...
        with col_b:
            ...
            st.caption(
                "How confident the model was in each score it gave. "
                "Higher confidence = more certain. "
                "Low confidence on incorrect scores can indicate model drift."
            )
        ...
        st.caption(
            "Full list of updates scored by the model in this period, sorted most recent first. "
            "Columns: content ID, document title, industry, vertical, "
            "model score, confidence score, confidence in plain English, model used, and time processed."
        )
        st.dataframe(
            view[[
